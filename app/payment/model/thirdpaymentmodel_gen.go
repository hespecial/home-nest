...
var (
	...
	cacheHomeNestPaymentThirdPaymentIdPrefix = "cache:homeNestPayment:thirdPayment:id:"
	cacheHomeNestPaymentThirdPaymentSnPrefix = "cache:homeNestPayment:thirdPayment:sn:"
)
...
func (m *defaultThirdPaymentModel) Delete(ctx context.Context, id int64) error {
	...
	homeNestPaymentThirdPaymentIdKey := fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentIdPrefix, id)
	homeNestPaymentThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentSnPrefix, data.Sn)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, homeNestPaymentThirdPaymentIdKey, homeNestPaymentThirdPaymentSnKey)
	return err
}
...
func (m *defaultThirdPaymentModel) FindOne(ctx context.Context, id int64) (*ThirdPayment, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, homeNestPaymentThirdPaymentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultThirdPaymentModel) FindOneBySn(ctx context.Context, sn string) (*ThirdPayment, error) {
	homeNestPaymentThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentSnPrefix, sn)
	var resp ThirdPayment
	err := m.QueryRowIndexCtx(ctx, &resp, homeNestPaymentThirdPaymentSnKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultThirdPaymentModel) Insert(ctx context.Context, data *ThirdPayment) (sql.Result, error) {
	homeNestPaymentThirdPaymentIdKey := fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentIdPrefix, data.Id)
	homeNestPaymentThirdPaymentSnKey := fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentSnPrefix, data.Sn)
...
func (m *defaultThirdPaymentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheHomeNestPaymentThirdPaymentIdPrefix, primary)
}
