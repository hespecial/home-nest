...
var (
	...
	cacheHomeNestTravelHomestayBusinessIdPrefix     = "cache:homeNestTravel:homestayBusiness:id:"
	cacheHomeNestTravelHomestayBusinessUserIdPrefix = "cache:homeNestTravel:homestayBusiness:userId:"
)
...
func (m *defaultHomestayBusinessModel) Delete(ctx context.Context, id int64) error {
	...
	homeNestTravelHomestayBusinessIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessIdPrefix, id)
	homeNestTravelHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessUserIdPrefix, data.UserId)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, homeNestTravelHomestayBusinessIdKey, homeNestTravelHomestayBusinessUserIdKey)
	return err
}
...
func (m *defaultHomestayBusinessModel) FindOne(ctx context.Context, id int64) (*HomestayBusiness, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, homeNestTravelHomestayBusinessIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayBusinessModel) FindOneByUserId(ctx context.Context, userId int64) (*HomestayBusiness, error) {
	homeNestTravelHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessUserIdPrefix, userId)
	var resp HomestayBusiness
	err := m.QueryRowIndexCtx(ctx, &resp, homeNestTravelHomestayBusinessUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultHomestayBusinessModel) Insert(ctx context.Context, data *HomestayBusiness) (sql.Result, error) {
	homeNestTravelHomestayBusinessIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessIdPrefix, data.Id)
	homeNestTravelHomestayBusinessUserIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessUserIdPrefix, data.UserId)
...
func (m *defaultHomestayBusinessModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayBusinessIdPrefix, primary)
}
