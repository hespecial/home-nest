...
var (
	...
	cacheHomeNestTravelHomestayCommentIdPrefix = "cache:homeNestTravel:homestayComment:id:"
)
...
func (m *defaultHomestayCommentModel) Delete(ctx context.Context, id int64) error {
	homeNestTravelHomestayCommentIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayCommentIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, homeNestTravelHomestayCommentIdKey)
	return err
}
...
func (m *defaultHomestayCommentModel) FindOne(ctx context.Context, id int64) (*HomestayComment, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, homeNestTravelHomestayCommentIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayCommentModel) Insert(ctx context.Context, data *HomestayComment) (sql.Result, error) {
	homeNestTravelHomestayCommentIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayCommentIdPrefix, data.Id)
...
func (m *defaultHomestayCommentModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayCommentIdPrefix, primary)
}
