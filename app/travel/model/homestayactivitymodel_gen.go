...
var (
	...
	cacheHomeNestTravelHomestayActivityIdPrefix = "cache:homeNestTravel:homestayActivity:id:"
)
...
func (m *defaultHomestayActivityModel) Delete(ctx context.Context, id int64) error {
	homeNestTravelHomestayActivityIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayActivityIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, homeNestTravelHomestayActivityIdKey)
	return err
}
...
func (m *defaultHomestayActivityModel) FindOne(ctx context.Context, id int64) (*HomestayActivity, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, homeNestTravelHomestayActivityIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayActivityModel) Insert(ctx context.Context, data *HomestayActivity) (sql.Result, error) {
	homeNestTravelHomestayActivityIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayActivityIdPrefix, data.Id)
...
func (m *defaultHomestayActivityModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayActivityIdPrefix, primary)
}
