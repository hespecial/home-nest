...
var (
	...
	cacheHomeNestTravelHomestayIdPrefix = "cache:homeNestTravel:homestay:id:"
)
...
func (m *defaultHomestayModel) Delete(ctx context.Context, id int64) error {
	homeNestTravelHomestayIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, homeNestTravelHomestayIdKey)
	return err
}
...
func (m *defaultHomestayModel) FindOne(ctx context.Context, id int64) (*Homestay, error) {
	homeNestTravelHomestayIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayIdPrefix, id)
	var resp Homestay
	err := m.QueryRowCtx(ctx, &resp, homeNestTravelHomestayIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultHomestayModel) Insert(ctx context.Context, data *Homestay) (sql.Result, error) {
	homeNestTravelHomestayIdKey := fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayIdPrefix, data.Id)
...
func (m *defaultHomestayModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheHomeNestTravelHomestayIdPrefix, primary)
}
